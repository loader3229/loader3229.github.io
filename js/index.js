 ...
 *  n(x) is for Decimal, EN(x) is for ExpantaNum, EN is defined in expantanum.js
 */
var n = function(x){return new Decimal(x);};

var decimalZero = n(0);
var decimalOne = n(1);
var expantaNumZero = EN(0);
var expantaNumOne = EN(1);

/** Page Open Count Update */
function pageOpenCountUpdate(){
...
/** Number Formatting */
var numberFormatting=document.createElement('script');
numberFormatting.src='/js/NumberFormatting.js';
document.head.appendChild(numberFormatting);
...
(function () {
	let games = [
		...
		},
		{
			...
			getInGamePoints(a) {
				let tmp = 0;
				JSON.parse(atob(atob(a).split(',')[12])).filter(function (a) { a.filter(function (b) { if (Number.isFinite(b)) tmp += b; }) });
				return tmp;
			},
			displayInGamePoints(a) {
				return a;
		...
		{
			id: "luck",
			location: "luck_incremental_save",
			getInGamePoints(a) {
	...
	window.update_total_points = function (metabonus) {
		...
		for (let i = 0; i < games.length; i++) {
			...
			try {
				if (games[i].currentSave != localStorage[games[i].location]){
					games[i].currentSave = localStorage[games[i].location];
...
metagame.src='/js/metagame.js';
document.head.appendChild(metagame);
...
setInterval(function(){
	...
	pl=pl*2+1;
	pldisp.innerHTML="Homepage Level "+Math.floor(pl)+", EXP:"+Math.floor(((Math.log10(parseFloat(localStorage.pageopencount)/5+10)-1)*1.25+Math.min(Math.log10(parseFloat(localStorage.kasumiIntimacy)/1000+1),9)+Math.log10(update_total_points(true)/10+1)+(getTotalRating()-1777)/100+((sha512_256(localStorage.supporterCode+"loader3229").slice(2) == '71fcb6c48d87276cfcaf7db32358649f23c82461d543509061a0e783f04be5')?1:0))*100)+"/"+Math.floor(((Math.floor(pl)/2)**(2/3))*100);
